...
<script>
import mIdex from "@/views/Oracle/Home/cptNode/index3.js";
import {
  queryDGState,
  queryDGRole,
  queryInstanceState,
...
export default {
  ...
  methods: {
    ...
    queryDGState (val) {
      ...
      queryASMInstanceState({ id: val }).then((result) => {
        if (result.data != 'TRUE') {
    ...
    queryDGRole (val) {
      ...
      queryTPS({ id: val }).then((result) => {
        that.copyItems1[1].bot = result.data + '/s' || '0/s'
      }).catch((err) => {
    ...
    queryREDO (val) {
      ...
      queryREDO({ id: val }).then((result) => {
        that.copyItems1[6].bot = result.data + '/s' || '0kb/s'
      }).catch((err) => {
    ...
    queryGAPState (val) {
      ...
      queryIOPS({ id: val }).then((result) => {
        let data = JSON.parse(result.data)
        that.copyItems1[7].bot = '读/写：' + data.READ + '/' + data.WRITE
      }).catch((err) => {
    ...
    queryDGArchiveLog (val) {
      ...
      queryConnectionNumber({ id: val }).then((result) => {
        let data = JSON.parse(result.data)
        that.copyItems1[8].bot = data[0].TOTAL_SESSIONS || 0
      }).catch((err) => {
