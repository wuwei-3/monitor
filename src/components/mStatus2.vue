...
<script>
import mIdex from "@/views/Oracle/Home/cptNode/index2.js";
import {
  queryProtectionMode,
  queryDGRole,
  queryInstanceState,
...
export default {
  ...
  methods: {
    ...
    // 保護模式
    queryProtectionMode (val) {
      let that = this
      that.copyItems1[0] = mIdex.instanSuc
      queryProtectionMode({ id: val }).then((result) => {
        that.copyItems1[0].bot = result.data[0].PROTECTION_MODE
      }).catch((err) => {
    ...
    queryMRPState (val) {
      ...
      queryMRPState({ id: val }).then((result) => {
        if (result.data != 'TRUE') {
