    ...
    <div class="e-top">
      ...
      <div class="r10 w100">
        <el-input v-model="file.rowNumber"
                  placeholder="请输入行数"
                  @change="rowNumberChange"></el-input>
      </div>
      <div class="p-select">
...
import { queryTraceFileContent } from "@/api/rac";
  ...
  methods: {
    ...
    search () {
      ...
      if (!this.file.fileName) {
        this.loading = false;
        return this.$message.error("请选择文件名");
      }
      let num = Number(this.file.rowNumber)
      if (this.$_.isNaN(num)) {
        return this.$message.warning('行数为整数')
      }
