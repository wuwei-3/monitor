...
import { queryDataDictionary } from "@/api/sqlMonitor";
export default {
  components: {
    aSelet,
    nTable,
  ...
  data () {
    return {
      ...
      // 统一表格样式
      column: [
        { label: "PARAMETER", prop: "PARAMETER" },
        { label: "SUM(GETS)", prop: "GETS" },
        { label: "SUM(GETMISSES)", prop: "GETMISSES" },
        { label: "Hit Ratio", prop: "Hit_Ratio" },
        { label: "SUM(UPDATES)", prop: "UPDATES" }
      ],
      tableData: [],
      dbStatus: false,
      loading: false,
