...
import { collectUserStatistics } from "@/api/sqlMonitor";
...
export default {
  ...
  methods: {
    ...
    search () {
      ...
      if (!this.file.degree) {
        return this.$message.error("请输入degree");
      }
      ...
      if (!this.file.estimatePercent) {
        return this.$message.error("请输入ESTIMATE PERCENT");
      }
