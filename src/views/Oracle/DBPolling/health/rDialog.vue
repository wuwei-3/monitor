    ...
    <el-dialog :visible.sync="visible"
      ...
      <div class="g-main">
        ...
        <el-form :model="ruleForm"
          ...
          <div class="n-body">
            <div class="y-span"><span>parallel</span></div>
            <div class="y-put">
              <el-form-item prop="parallel">
                <el-input v-model="ruleForm.parallel"></el-input>
              </el-form-item>
            </div>
          </div>
        </el-form>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loading: false,
      rules: {
        parallel: [{ validator: checkAge, trigger: "change" }]
      },
      ruleForm: {
        id: '',
        userName: "",
  ...
  methods: {
    ...
    submitForm (formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (!this.ruleForm.parallel) {
            return this.$message.error('请选择parallel');
          }
          // 处理数据保存d
          this.loading = true
          reBuildIndex(this.ruleForm)
            .then(result => {
