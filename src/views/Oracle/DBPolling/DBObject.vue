...
import { queryDBObject } from "@/api/DBPolling";
...
export default {
  ...
  data () {
    return {
      ...
      // 统一表格样式
      column: [
        { label: "USERNAME", prop: "key" },
        { label: "OBJECT_TYPE", prop: "下层key" },
        { label: "TOTAL_NUM", prop: "下层value" },
      ],
      column1: [
        { label: "USERNAME", prop: "OWNER" },
        { label: "", prop: "OBJECT_TYPE" },
        { label: "", prop: "VALUE" },
      ],
      tableData: [],
      dbStatus: false,
    ...
    dbDate (val) {
      ...
      val.forEach((item, index) => {
        let par = {
          key: item.OWNER,
          column: [
            { label: item.OWNER, prop: "OBJECT_TYPE" },
            { label: "", prop: "VALUE" },
          ],
