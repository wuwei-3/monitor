  ...
  <div class="r-role">
    ...
    <div v-if="delVisable">
      <a-delete :visible="delVisable"
                ...
                :status="6"
                :title="'删除告警级别'"
                :content="delName.length  == tableData.length?'确定要删除全部告警级别配置？':'确当要删除告警级别配置？'"></a-delete>
    </div>
  </div>
</template>
...
import { warnList } from "@/api/Oracle.js";
...
export default {
  ...
  data () {
    return {
      ...
      // 统一表格样式
      column: [
        { label: "级别名称", prop: "name" },
        { label: "顺序值", prop: "sort" },
        { label: "展开样式", prop: "fontClass" },
