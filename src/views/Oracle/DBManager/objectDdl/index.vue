<template>
  ...
  <div class="objectsize">
    <div class="e-top">
      ...
      <div class="w100 r10">
        <a-selet4 :options="reportOption2"
                  @getOPtions="getOPtions2"
                  :title="'请选择对象类型'"
                  :reportId="file.objectType"></a-selet4>
      </div>
      <div class="w100 r10">
      ...
      <div class="w100 r10">
        <el-input :value="file.objectName"
                  placeholder="请选择对象名"
                  clearable
...
import { queryObjectDDL } from "@/api/sqlManager";
...
export default {
  ...
  methods: {
    // 
    getOPtions2 (val) {
      this.file.objectType = val
    },
    getOPtions1 (val) {
      this.file.dbName = val
      this.file.objectName = ""
      this.file.userName = ""
      this.file.objectType = 'INDEX'
    },
    // 获取用户名 表名
    getValue (val, val2) {
      if (val2 == 1) {
        ...
        this.Visible = false;
        this.file.objectName = val.OBJECT_NAME
      }
    },
    confirmAwr () {
      this.Visible = false;
    ...
    tableNameAwr () {
      ...
      if (!this.file.objectType) {
        return this.$message.error("请选择对象类型");
      }
      this.Visible = true;
      this.sqlTitle = "表名";
    },
    ...
    search () {
      ...
      if (!this.file.objectName) {
        return this.$message.error("请选择对象名");
      }
