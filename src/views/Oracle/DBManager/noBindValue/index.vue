<template>
  ...
  <div class="nobindvalue">
    <div class="e-top">
      ...
      <div class="r10">可共享游标数</div>
      <div class="w100 r10">
        <el-input v-model="file.shareCursor"
                  placeholder="可共享游标数"
                  clearable></el-input>
      </div>
      <div class="r10">行数</div>
      <div class="w100 r10">
...
import { queryExecutionPlan, download } from "@/api/common";
import { queryUnboundSQL } from "@/api/sqlManager";
...
export default {
  ...
  data () {
    return {
      ...
      column: [
        ...
        { label: "FORCE_MATCHING_SIGNATURE", prop: "FORCE_MATCHING_SIGNATURE" },
        { label: "UNSHARED_COUNT", prop: "UNSHARED_COUNT" },
  ...
  methods: {
    ...
    search () {
      ...
      if (!this.file.shareCursor) {
        return this.$message.error("请输入可共享游标数");
      }
      ...
      let num = Number(this.file.shareCursor)
      if (this.$_.isNaN(num)) {
        return this.$message.warning('请输入数字')
      }
      let num1 = Number(this.file.rowNumber)
      if (this.$_.isNaN(num1)) {
        return this.$message.warning('请输入数字')
      }
