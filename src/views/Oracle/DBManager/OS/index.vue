...
import { queryOSKernelParameterList } from "@/api/sqlManager";
export default {
  components: {
    aSelet,
    nTable,
  ...
  data () {
    return {
      ...
      column: [
        ...
        { label: "PARA_NAME", prop: "PARA_NAME" },
        { label: "PARA_VALUE", prop: "PARA_VALUE" },
        { label: "ADVICE_VALUE", prop: "ADVICE_VALUE" },
      ],
      tableData: [],
      loading: false,
      file: {
  ...
  methods: {
    ...
    getMes (val) {
      this.loading = true;
      let _self = this;
      queryOSKernelParameterList(val)
        .then(result => {
