    ...
    <div class="e-top">
      ...
      <div class="w100 r10">
        <a-selet4 :options="reportOption2"
                  ...
                  :title="'压缩'"
                  :reportId="file.inmemoryCompression"></a-selet4>
      </div>
      <div class="w100 r10">
        <a-selet4 :options="reportOption3"
                  @getOPtions="getOPtions3"
                  :title="'优先级'"
                  :reportId="file.inmemoryPriority"></a-selet4>
      </div>
      <div class="w100 r10">
...
import { tableEnableIM } from "@/api/sqlManager";
...
export default {
  ...
  methods: {
    getOPtions1 (val) {
      ...
      this.file.tableName = ""
      this.file.userName = ""
      this.file.inmemoryCompression = ''
      this.file.inmemoryPriority = ''
    },
    getOPtions2 (val) {
      this.file.inmemoryCompression = val
    },
    getOPtions3 (val) {
      this.file.inmemoryPriority = val
    },
    // 获取用户名 表名
    getValue (val, val2) {
      if (val2 == 1) {
    ...
    search () {
      ...
      if (!this.file.inmemoryCompression) {
        return this.$message.error("请选择压缩");
      }
      if (!this.file.inmemoryPriority) {
