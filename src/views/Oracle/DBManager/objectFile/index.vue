<template>
  ...
  <div class="objectfile">
    <div class="e-top">
      ...
      <div class="w100 r10">
        <el-input :value="file.fileId"
                  placeholder="数据文件编号及名称"
                  clearable
                  @focus="userNameAwr"></el-input>
      </div>
      <div class="w100 r10">
        <el-input :value="file.segmentObjectType"
                  placeholder="段对象类型"
                  clearable
...
import { queryPartitionTableInfo } from "@/api/sqlManager";
...
export default {
  ...
  data () {
    return {
      ...
      column: [
        ...
        { label: "SEGMENT_TYPE", prop: "SEGMENT_TYPE" },
        { label: "EXTENT_ID", prop: "EXTENT_ID" },
        { label: "FILE_ID", prop: "FILE_ID" },
  ...
  methods: {
    ...
    getValue (val, val2) {
      if (val2 == 1) {
        this.Visible = false;
        this.file.fileId = val.FILE_ID
      } else {
        this.Visible = false;
        this.file.segmentObjectType = val.SEGMENT_TYPE
      }
    },
    confirmAwr () {
      this.Visible = false;
    ...
    search () {
      ...
      if (!this.file.fileId) {
        return this.$message.error("请选择数据文件编号");
      }
      if (!this.file.segmentObjectType) {
        return this.$message.error("请选择段对象类型");
      }
