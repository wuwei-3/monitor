...
import { queryMonitoredIndex, cancelMonitorIndex } from "@/api/sqlManager";
...
export default {
  ...
  data () {
    return {
      ...
      column: [
        ...
        { label: "MONITORING", prop: "MONITORING" },
        { label: "USED", prop: "USED" },
        { label: "START_MONITORING", prop: "START_MONITORING" },
        { label: "END_MONITORING", prop: "END_MONITORING" },
        { label: "操作", prop: "operaton5", type: 1 }
      ],
      tableData: [],
      dbStatus: false,
      loading: false,
  ...
  methods: {
    ...
    // 监控索引
    cancelMonitorIndex (val) {
      if (val.MONITORING == 'NO') {
        return this.$message.error('当前索引已取消，请勿重复操作')
      }
      let par = {
        id: this.file.id,
