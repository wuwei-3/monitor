  ...
  <div class="resourcedialog">
    <el-dialog :visible.sync="visible"
      ...
      <div class="g-main">
        ...
        <div class="n-body">
          <div class="y-span"><span>资源计划名称</span></div>
          <div class="y-put">
            <el-input v-model="file.planName"
                      disabled=""></el-input>
          </div>
        </div>
...
import { createPDBResourcePlanDirectives, modifyPDBResourcePlanDirectives } from "@/api/sqlManager";
export default {
  components: {
    aSelect
  },
  ...
  mounted () {
    this.ruleForm.planName = this.file.planName
    this.items = JSON.parse(this.update.resourcePlanRows)
  },
  methods: {
    // 
    deleteIndex () {
    ...
    submitForm (formName) {
      ...
      if (this.update.resourcePlanRows) {
        let par = {
          id: this.file.id,
          planName: this.ruleForm.planName,
          resourcePlanRows: JSON.stringify(this.items),
        }
        modifyPDBResourcePlanDirectives(par).then((result) => {
        ...
        createPDBResourcePlanDirectives(par).then((result) => {
          this.$emit("cancel", true);
          this.$message.success(result.msg)
          this.loading = false
        }).catch((err) => {
